...
def test_resource_monitor():
    pkg_resources_file = join(
        dirname(dirname(abspath(__file__))), "tests", "package_resources.json"
    )
    stats = {}
    ...
    for builders in [1, 2, 4, 8, 16]:
        ...
        for res in init_resources:
            assert (
                scheduler.resourceManager.machineResources[res] == init_resources[res]
            )
