...
CORRECT_BEHAVIOR = [
    (
        (),
        "build --force-unknown-architecture zlib",
        [("action", "build"), ("workDir", "sw"), ("referenceSources", "sw/MIRROR")],
    ),
    (
        (),
        "init",
        [("action", "init"), ("workDir", "sw"), ("referenceSources", "sw/MIRROR")],
    ),
    ((), "version", [("action", "version")]),
    ((), "clean", [("action", "clean"), ("workDir", "sw")]),
    (
        (),
        "build --force-unknown-architecture -j 10 zlib",
        [("action", "build"), ("jobs", 10), ("pkgname", ["zlib"])],
    ),
    (
        (),
        "build --force-unknown-architecture -j 10 zlib --disable gcc --disable foo",
        [("disable", ["gcc", "foo"])],
    ),
    (
        (),
        "build --force-unknown-architecture -j 10 zlib --disable gcc --disable foo,bar",
        [("disable", ["gcc", "foo", "bar"])],
    ),
    (
        (),
        "init zlib --dist master",
        [("dist", {"repo": "alisw/alidist", "ver": "master"})],
    ),
    (
        (),
        "init zlib --dist ktf/alidist@dev",
        [("dist", {"repo": "ktf/alidist", "ver": "dev"})],
    ),
    (
        (),
        "build --force-unknown-architecture zlib --remote-store rsync://test.local/",
        [("noSystem", "*"), ("remoteStore", "rsync://test.local/")],
    ),
    (
        (),
        "build --force-unknown-architecture zlib --remote-store rsync://test.local/::rw",
        [
            ("noSystem", "*"),
            ("remoteStore", "rsync://test.local/"),
            ("writeStore", "rsync://test.local/"),
        ],
    ),
    (
        (),
        "build --force-unknown-architecture zlib --no-remote-store --remote-store rsync://test.local/",
        [("noSystem", None), ("remoteStore", "")],
    ),
    (
        (),
        "build zlib --architecture slc7_x86-64",
        [
            ("noSystem", "*"),
            ("preferSystem", False),
            ("remoteStore", "https://s3.cern.ch/swift/v1/alibuild-repo"),
        ],
    ),
    (
        (),
        "build zlib --architecture ubuntu1804_x86-64",
        [("noSystem", None), ("preferSystem", False), ("remoteStore", "")],
    ),
    (
        (),
        "build zlib -a slc7_x86-64",
        [
            ("docker", False),
            ("dockerImage", None),
    ...
    (
        (),
        "build zlib -a slc7_x86-64 --docker-image registry.cern.ch/alisw/some-builder",
        [("docker", True), ("dockerImage", "registry.cern.ch/alisw/some-builder")],
    ),
    (
        (),
        "build zlib -a slc7_x86-64 --docker",
        [("docker", True), ("dockerImage", "registry.cern.ch/alisw/slc7-builder")],
    ),
    (
        (),
        "build zlib -a slc7_x86-64 --docker-extra-args=--foo",
        [
            ("docker", True),
            ("dockerImage", "registry.cern.ch/alisw/slc7-builder"),
            ("docker_extra_args", ["--foo", "--network=host"]),
        ],
    ),
    (
        (),
        "build zlib --devel-prefix -a slc7_x86-64 --docker",
        [
            ("docker", True),
            ("dockerImage", "registry.cern.ch/alisw/slc7-builder"),
            ("develPrefix", "%s-slc7_x86-64" % os.path.basename(os.getcwd())),
        ],
    ),
    (
        (),
        "build zlib --devel-prefix -a slc7_x86-64 --docker-image someimage",
        [
            ("docker", True),
    ...
    (
        (),
        "--debug build --force-unknown-architecture --defaults o2 O2",
        [
            ("debug", True),
            ("action", "build"),
            ("defaults", ["o2"]),
    ...
    (
        (),
        "build --force-unknown-architecture --debug --defaults o2 O2",
        [
            ("debug", True),
            ("action", "build"),
            ("force_rebuild", []),
    ...
    (
        (),
        "build --force-unknown-architecture --force-rebuild O2 --force-rebuild O2Physics --defaults o2 O2Physics",
        [
            ("action", "build"),
            ("force_rebuild", ["O2", "O2Physics"]),
            ("defaults", ["o2"]),
            ("pkgname", ["O2Physics"]),
        ],
    ),
    (
        (),
        "build --force-unknown-architecture --force-rebuild O2,O2Physics --defaults o2 O2Physics",
        [
            ("action", "build"),
            ("force_rebuild", ["O2", "O2Physics"]),
            ("defaults", ["o2"]),
    ...
    (
        ...
        [
            ("action", "init"),
            ("workDir", "sw3"),
            ("referenceSources", "sw3/MIRROR"),
            ("chdir", "mydir"),
        ],
    ),
    (
        ("sw", "."),
        "clean --chdir mydir2 --work-dir sw4",
        [("action", "clean"), ("workDir", "sw4"), ("chdir", "mydir2")],
    ),
    (
        (),
        "doctor zlib -C mydir -w sw2",
        [("action", "doctor"), ("workDir", "sw2"), ("chdir", "mydir")],
    ),
    (
        (),
        "deps zlib --outgraph graph.pdf",
        [("action", "deps"), ("outgraph", "graph.pdf")],
    ),
]

GETSTATUSOUTPUT_MOCKS = {"which docker": (0, "/usr/local/bin/docker")}


class ArgsTestCase(unittest.TestCase):
    @mock.patch(
        "bits_helpers.utilities.getoutput", new=lambda cmd: "x86_64"
    )  # for uname -m
    ...
    def test_actionParsing(self, mock_commands):
        ...
        for env, cmd, effects in CORRECT_BEHAVIOR:
            (bits_helpers.args.DEFAULT_WORK_DIR, bits_helpers.args.DEFAULT_CHDIR) = (
                env or ("sw", ".")
            )
