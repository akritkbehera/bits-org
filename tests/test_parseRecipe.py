...
class TestRecipes(unittest.TestCase):
    ...
    def test_parseDefaults(self) -> None:
        ...
        self.assertEqual(
            overrides, {"defaults-release": {}, "root": {"requires": "GCC"}}
        )
    ...
    def test_validateDefault(self) -> None:
        ...
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": ["o2", "o2-dataflow"]}, "release"
        )
        self.assertEqual(ok, False)
        self.assertEqual(
            out,
            "Cannot compile foo with `release' default. Valid defaults are\n - o2\n - o2-dataflow",
        )
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": ["o2", "o2-dataflow"]}, "o2"
        )
        self.assertEqual(ok, True)
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": "o2-dataflow"}, "o2"
        )
        self.assertEqual(ok, False)
        self.assertEqual(validDefaults, ["o2-dataflow"])
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": "o2"}, "o2"
        )
        self.assertEqual(ok, True)
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": 1}, "o2"
        )
        self.assertEqual(ok, False)
        self.assertEqual(
            out, "valid_defaults needs to be a string or a list of strings. Found [1]."
        )
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": {}}, "o2"
        )
        self.assertEqual(ok, False)
        self.assertEqual(
            out, "valid_defaults needs to be a string or a list of strings. Found [{}]."
        )
