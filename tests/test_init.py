...
CLONE_EVERYTHING = [
    ...
    call(
        ("remote", "set-url", "--push", "upstream", "https://github.com/alisw/AliRoot"),
        directory="./AliRoot",
    ),
]
...
class InitTestCase(unittest.TestCase):
    ...
    def test_doRealInit(
        ...
        mock_open.side_effect = lambda x: {
            "/alidist/defaults-release.sh": StringIO(
                "package: defaults-release\nversion: v1\n---"
            ),
            "/alidist/aliroot.sh": StringIO(
        ...
        mock_path.exists.assert_has_calls(
            [call("."), call("/sw/MIRROR"), call("/alidist"), call("./AliRoot")]
        )
