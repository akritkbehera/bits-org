...
class ResourceManager(object):
        ...
        for xtype in self.esStats["packages"]:
            for pkg in self.esStats["packages"][xtype]:
                for res in self.resouceList:
                    if (
                        self.esStats["packages"][xtype][pkg][res]
                        > self.machineResources[res]
                    ):
                        self.esStats["packages"][xtype][pkg][res] = (
                            self.machineResources[res]
                        )
    ...
    def allocResourcesForExternals(
        self, externalsList, count=1000
    ):  # return ordered list for externals that can be started
        externals_to_run = []
        if count <= 0:
        ...
        for ext_full in externalsList:
            ...
            build_type = (
                ext_items[0]
                if ext_items[0] in ["prep", "build", "install", "srpm", "rpms"]
                else "build"
            )
            ...
            if ext_full in self.seenPackages:
                ...
                if ext not in pkg_stats:
                    ...
                    self.scheduler.debug(
                        "New external found, creating default entry %s" % stats
                    )
                else:
                    for k in self.esStats["defaults"]:
        ...
        for ex_stats in externalsList_sorted:
            if not [
                r for r in self.resouceList if ex_stats[r] > self.machineResources[r]
            ]:
