...
def resolve_spec_data(spec, data, defaults, branch_basename="", branch_stream=""):
    ...
    """
    defaults_upper = (
        "" if defaults == ["release"] else "_".join(d.upper() for d in defaults)
    )
    ...
    try:
        ...
        if spec and "from" in spec:
            ...
            if spec["from"] in visited:
                raise RuntimeError(
                    f" Cyclic Dependency: {' -> '.join(list(visited) + [spec['from']])}"
                )
            visited.append(spec["from"])
            parent_dir = os.path.join(repoDir, spec["from"])
            base_filename, pkgdir = resolveFilename(
                {}, filename, parent_dir, generatePackages
            )
            base_reader = getRecipeReader(
                base_filename, repoDir, generatePackages[parent_dir]
            )
            err, base_spec, base_recipe = parseRecipe(
                base_reader, generatePackages, visited
            )
    ...
    while packages:
        ...
        for d in defaults:
            if p == "defaults-release" and ("defaults-" + d) in specs:
                ...
                pkg_filename = (
                    ("defaults-" + d) if p == "defaults-release" else p.lower()
                )
        if skip:
            continue
        ...
        err, spec, recipe = parseRecipe(
            getRecipeReader(filename, configDir, generatedPackages[pkgdir]),
            generatedPackages,
        )
        dieOnError(err, err)
