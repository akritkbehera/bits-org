...
def deps_graph(specs, package, runtime_only=False):
  """Return {pkg: [deps]} for *package*'s dependency tree, alphabetically sorted.
  Every package in the tree gets an entry, with its direct prerequisites also
  alphabetically sorted. With *runtime_only*, ``build_requires`` edges are not followed, so
  packages needed only to build something are left out.
  No installation order is calculated; consumers can derive one from the edges.
  """
  def deps_of(p):
    spec = specs[p]
    if not runtime_only:
...
def deps_makefile(specs, package, runtime_only=False, header=True):
  ...
  *header* adds the "generated by" comment line, which suits a file written to
  ...
  # Makefile output retains dependency-first ordering. Embedded metadata only
  # needs the graph, so it does not pay for this sort once per package.
  local_specs = {p: {"package": p, "requires": deps} for p, deps in graph.items()}
  order = list(topological_sort(local_specs, alphabetical=True))
