...
def doDeps(args, parser):
    ...
    for s in specs.values():
        ...
        s["requires"] = [
            r
            for r in s["requires"]
            if r not in args.disable and r != "defaults-release"
        ]
        s["build_requires"] = [
            r
    ...
    for k, spec in specs.items():
        ...
        # Node definition
        dot += (
            '"%s" [shape=box, style="rounded,filled", fontname="helvetica", fillcolor=%s]\n'
            % (k, color)
        )
    ...
    try:
        ...
        dieOnError(
            True,
            "Could not find dot in PATH. Please install graphviz and add it to PATH.",
        )
    try:
        if args.neat:
            execute(
                "tred {dotFile} > {dotFile}.0 && mv {dotFile}.0 {dotFile}".format(
                    dotFile=fp.name
                )
            )
