...
from bits_helpers.utilities import getPackageList, parseDefaults, readDefaults, validateDefaults, resolve_variables, incompatibleFlavorDefaults, topological_sort


def deps_graph(specs, package, runtime_only=False):
  """Return {pkg: [deps]} for *package*'s dependency tree, in build order.

  Every package in the tree gets an entry, with its prerequisites also in build
  order. With *runtime_only*, ``build_requires`` edges are not followed, so
  ...
  for p in build_order:
    ...
    graph[p] = sorted(deps, key=position.get)
  return graph
...
def deps_makefile(specs, package, runtime_only=False, header=True):
  ...
  *header* adds the "generated by" comment line, which suits a file written to
  ...
  for p, deps in deps_graph(specs, package, runtime_only).items():
