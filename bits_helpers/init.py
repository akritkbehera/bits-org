...
def doInit(args):
    ...
    if path.exists(args.configDir):
        ...
        cmd = [
            ...
            "upstream",
            args.dist["repo"]
            if ":" in args.dist["repo"]
            else "https://github.com/" + args.dist["repo"],
        ]
        if args.dist["ver"]:
    ...
    defaultsReader = lambda: readDefaults(
        args.configDir, args.defaults, lambda msg: error("%s", msg), args.architecture
    )
    ...
    for p in pkgs:
        ...
        dieOnError(
            not writeRepo,
            "package %s has no source field and cannot be developed" % spec["package"],
        )
        if path.exists(dest):
        ...
        debug(
            "cloning %s%s for development",
            spec["package"],
            " version " + p["ver"] if p["ver"] else "",
        )

        updateReferenceRepoSpec(
            args.referenceSources, spec["package"], spec, True, False
        )
        ...
        refObjects = os.path.join(
            os.path.realpath(args.referenceSources), spec["package"].lower(), "objects"
        )
    ...
    banner(
        "Development directory %s created%s",
        args.develPrefix,
