    ...
    def format(self, record):
        ...
        return "\n".join(
            ...
            % {
                ...
                "levelname": (
                    self.LEVEL_COLORS.get(record.levelno, self.COLOR_RESET)
                    + record.levelname
                    + self.COLOR_RESET
                ),
                "message": x,
    ...
    logger_handler.setFormatter(
        LogFormatter(
            "%(asctime)s:%(levelname)s:{}:{}: %(message)s".format(
                main_package, short_version
            )
            if package is None
            else "%(asctime)s:%(levelname)s:{}:{}:{}: %(message)s".format(
                main_package, package, short_version
            )
        )
    )
...
class ProgressPrint:
    def __init__(self, begin_msg="", min_interval=0.0) -> None:
        self.count = -1
        self.lasttime = 0
        self.STAGES = ".", "..", "...", "....", ".....", "....", "...", ".."
    ...
    def end(self, msg="", error=False):
        ...
        if msg:
            sys.stderr.write(
                ": %s%s\033[m" % ("\033[31m" if error else "\033[32m", msg)
            )
        sys.stderr.write("\n")
        sys.stderr.flush()
