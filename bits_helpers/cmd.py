...
def getstatusoutput(command, timeout=None, cwd=None):
    ...
    proc = Popen(
        command, shell=isinstance(command, str), stdout=PIPE, stderr=STDOUT, cwd=cwd
    )
    try:
...
class DockerRunner:
    ...
    def __init__(
        self, docker_image, docker_run_args=(), extra_env={}, extra_volumes=[]
    ) -> None:
        ...
        def getstatusoutput_docker(cmd, cwd=None):
            if self._container is None:
                ...
                return getstatusoutput(
                    "{}{} -c {}".format(command_prefix, BASH, quote(cmd)), cwd=cwd
                )
            envOpts = []
            for env in self._extra_env.items():
                envOpts.append("-e")
