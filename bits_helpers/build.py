...
def update_git_repos(args, specs, buildOrder):
    ...
    def update_repo(package, git_prompt):
        ...
        output = logged_scm(
            ...
            package,
            args.referenceSources,
            specs[package]["scm"].listRefsCmd(
                specs[package].get("reference", specs[package]["source"])
            ),
            ".",
            prompt=git_prompt,
            logOutput=False,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_download)):
            ...
            try:
                ...
                dieOnError(
                    True, "Error on fetching %r: %s. Aborting." % (futurePackage, exc)
                )
            else:
                debug(
                    "%r package updated: %d refs found",
                    futurePackage,
    ...
    for package in requires_auth:
        banner(
            "If prompted now, enter your username and password for %s below\n"
...
def createDistLinks(spec, specs, args, syncHelper, repoType, requiresType):
    ...
    target_dir = (
        "{work_dir}/TARS/{arch}/{repo}/{package}/{package}-{version}-{revision}".format(
            work_dir=args.workDir, arch=args.architecture, repo=repoType, **spec
        )
    )
    shutil.rmtree(target_dir.encode("utf-8"), ignore_errors=True)
    makedirs(target_dir, exist_ok=True)
    for pkg in [spec["package"]] + list(spec[requiresType]):
        dep_tarball = "../../../../../TARS/{arch}/store/{short_hash}/{hash}/{package}-{version}-{revision}.{arch}.tar.gz".format(
            arch=args.architecture, short_hash=specs[pkg]["hash"][:2], **specs[pkg]
        )
...
def storeHashes(package, specs, considerRelocation):
    ...
    h_alternatives = [
        (spec.get("tag", "0"), spec["commit_hash"], h_default),
    ...
    for ref, git_hash in spec.get("scm_refs", {}).items():
        if ref.startswith("refs/tags/") and git_hash == real_commit_hash:
            ...
            debug(
                "Tag %s also points to %s, storing alternative",
                tag_name,
                real_commit_hash,
            )
            hasher = h_all.copy()
    ...
    for key in modifies_full_hash_dicts:
        if key not in spec:
            ...
            assert isinstance(spec[key], OrderedDict), "spec[%r] was of type %r" % (
                key,
                type(spec[key]),
            )
    ...
    spec["remote_hashes"] = [spec["remote_revision_hash"]] + list(
        {h.hexdigest() for _, _, h in h_alternatives} - {spec["remote_revision_hash"]}
    )
    ...
    spec["local_hashes"] = [spec["local_revision_hash"]] + list(
        {h.hexdigest() for _, _, h in h_alternatives} - {spec["local_revision_hash"]}
    )
...
def hash_local_changes(spec):
    ...
    if "track_env" in spec:
        assert isinstance(spec["track_env"], OrderedDict), "spec[%r] was of type %r" % (
            "track_env",
            type(spec["track_env"]),
        )
        ...
        h(
            ", ".join(
                ...
                # and on repr(["a", "b"]) being "['a', 'b']".
                "(%r, %r)" % (key, value)
                for key, value in spec["track_env"].items()
            )
        )
        h("])")
    ...
    try:
        ...
        warning(
            "You have untracked changes in %s, so bits cannot detect "
...
def better_tarball(spec, old, new):
    ...
    old_is_local, new_is_local = (
        old_rev.startswith("local"),
        new_rev.startswith("local"),
    )
...
def generate_initdotsh(package, specs, architecture, post_build=False):
    ...
    if post_build:
        ...
        for key in ("env", "append_path", "prepend_path"):
            dieOnError(
                not isinstance(spec.get(key, {}), dict),
        ...
        lines.extend(
            'export {}="{}"'.format(key, resolve_spec_data(spec, value, ""))
        ...
        lines.extend(
            'export {key}="${key}:{value}"'.format(
                key=key, value=":".join(asList(value))
            )
        ...
        prepend_path = {
            key: [resolve_spec_data(spec, dir, "") for dir in asList(value)]
        ...
        for key, value in (
            ...
            prepend_path.setdefault(key, []).insert(
                0, "${}_ROOT/{}".format(bigpackage, value)
            )
        lines.extend(
            '[ ! -d "{value}" ] || export {key}="{value}${{{key}+:${key}}}"'.format(
                key=key, value=dir
            )
...
def runBuildCommand(
    ...
    if args.builders == 1:
        ...
        scheduler.log(
            (
                "Unpacking %s@%s"
                if cachedTarball
                else "Compiling %s@%s (use --debug for full output)"
            )
            % (
                spec["package"],
                args.develPrefix
    ...
    if args.resourceMonitoring:
        err = run_monitor_on_command(
            build_command, "%s/%s.json" % (scriptDir, p), printer=progress
        )
    else:
    ...
    # Determine paths
    devSuffix = (
        "-" + args.develPrefix if "develPrefix" in args and spec["is_devel_pkg"] else ""
    )
    log_path = f"{buildWorkDir}/BUILD/{spec['package']}-latest{devSuffix}/log"
    build_dir = (
        f"{buildWorkDir}/BUILD/{spec['package']}-latest{devSuffix}/{spec['package']}"
    )
    ...
    if not args.debug:
        buildErrMsg += (
            f"  • Rebuild with debug:      bitsBuild build {spec['package']} --debug\n"
        )
    ...
    """)
        ...
        try:
            ...
            args_str = " ".join(
                f"--{k}={v}" for k, v in vars(args).items() if v and k in safe_args
            )
            detected_arch = detectArch()
      ...
      """)
            ...
            if detected_arch.startswith("osx"):
                buildErrMsg += (
                    f"XCode version: {getstatusoutput('xcodebuild -version')[1]}"
                )
...
def doFinalSync(spec, specs, args, syncHelper):
    ...
    createDistLinks(
        spec, specs, args, syncHelper, "dist-runtime", "full_runtime_requires"
    )
...
def doBuild(args, parser):
    ...
    defaultsReader = lambda: readDefaults(
    ...
    has_sapling = (config_path / ".sl").exists() or (
        config_path / ".git" / "sl"
    ).exists()
    ...
    debug(
        "Using bitsBuild from bits@%s recipes in dist@%s",
        __version__ or "unknown",
        os.environ["BITS_DIST_HASH"],
    )

    install_wrapper_script("git", workDir)

    extra_env = {
        "BITS_CONFIG_DIR": "/alidist"
        if args.docker
        else os.path.abspath(args.configDir)
    }
    ...
    with DockerRunner(
        ...
        extra_env=extra_env,
        extra_volumes=[f"{os.path.abspath(args.configDir)}:/alidist:ro"]
        if args.docker
        else [],
    ) as getstatusoutput_docker:
    ...
    if systemPackages:
        banner(
            "bits can take the following packages from the system and will not build them:\n  %s",
            ", ".join(systemPackages),
        )
        ...
        if len(builtPackages) > 1:
            banner(
                ...
                "\n - ".join(
                    ...
                    for x in builtPackages
                    if x != "defaults-release"
                ),
            )
        else:
    ...
    if develPkgs:
        banner(
            "You have packages in development mode (%s).\n"
    ...
    for pkg, spec in specs.items():
        ...
        if "source" in spec:
            ...
            has_sapling = (source_path / ".sl").exists() or (
                source_path / ".git" / "sl"
            ).exists()
            if has_sapling and shutil.which("sl"):
                use_sapling = True
    ...
    for p in buildOrder:
        ...
        dieOnError(
            "source" not in spec and spec["is_devel_pkg"],
            ...
            "{recipe}.sh instead.".format(package=p, recipe=p.lower()),
        )
        ...
        if "source" in spec:
            ...
            if spec["is_devel_pkg"]:
                ...
                spec["tag"] = (
                    args.develPrefix if "develPrefix" in args else develPackageBranch
                )
                spec["commit_hash"] = "0"
        ...
        spec["version"] = resolve_version(
            spec, args.defaults, branch_basename, branch_stream
        )
        ...
        for k, v in variables.items():
            variables[k] = resolve_spec_data(
                spec, v, args.defaults, branch_basename, branch_stream
            )
        if "source" in spec:
            spec["source"] = resolve_spec_data(
                spec, spec["source"], args.defaults, branch_basename, branch_stream
            )
        if "sources" in spec:
            spec["sources"] = [
                resolve_spec_data(
                    spec, src, args.defaults, branch_basename, branch_stream
                )
                for src in spec["sources"]
            ]
        if variables or spec.get("expand_recipe", False):
            spec["recipe"] = resolve_spec_data(
                spec, spec["recipe"], args.defaults, branch_basename, branch_stream
            )

        if (
            spec["is_devel_pkg"]
            and "develPrefix" in args
            and args.develPrefix != "ali-master"
        ):
    ...
    for p in buildOrder:
        ...
        for key in ("requires", "runtime_requires", "build_requires"):
            ...
            for dep in spec.get(key, ()):
                ...
                spec[full_key] |= specs[dep][
                    "full_requires" if key == "build_requires" else full_key
                ]
    ...
    if args.plugin != "legacy":
        return importlib.import_module(
            "bits_helpers.%s_plugin" % args.plugin
    ...
    report_event(
        "install",
        "{p} disabled={dis} devel={dev} system={sys} own={own} deps={deps}".format(
            p=args.pkgname,
    ...
    if (args.builders > 1) and buildOrder:
        ...
        scheduler = Scheduler(
            args.builders, logDelegate=logger, buildStats=args.resources
        )
    ...
    while buildOrder:
        ...
        debug(
            "develPkgs = %r",
            sorted(spec["package"] for spec in specs.values() if spec["is_devel_pkg"]),
        )
        storeHashes(p, specs, considerRelocation=args.architecture.startswith("osx"))
        debug(
            "Hashes for recipe %s are %s (remote); %s (local)",
            p,
            ", ".join(spec["remote_hashes"]),
            ", ".join(spec["local_hashes"]),
        )
        ...
        for symlink_path in packages:
            ...
            matcher = "../../{arch}/store/[0-9a-f]{{2}}/([0-9a-f]+)/{package}-{version}-((?:local)?[0-9]+).{arch}.tar.gz$".format(
            ...
            if not (
                ("local" in revision and rev_hash in spec["local_hashes"])
                or ("local" not in revision and rev_hash in spec["remote_hashes"])
            ):
                # This tarball's hash doesn't match what we need. Remember that its
                # revision number is taken, in case we assign our own later.
                if (
                    revision.startswith(revisionPrefix)
                    and revision[len(revisionPrefix) :].isdigit()
                ):
            ...
            # for reuse yet.
            candidate = better_tarball(
                spec, candidate, (revision, rev_hash, symlink_path)
            )
        ...
        try:
            ...
            if spec["is_devel_pkg"] and "incremental_recipe" in spec:
                ...
                call_ignoring_oserrors(
                    ...
                    "{wd}/{arch}/{package}/latest-{build_family}".format(
                        wd=workDir, arch=args.architecture, **spec
                    ),
                )
                call_ignoring_oserrors(
                    symlink,
                    "{version}-{revision}".format(**spec),
                    "{wd}/{arch}/{package}/latest".format(
                        wd=workDir, arch=args.architecture, **spec
                    ),
                )
        ...
        spec["old_devel_hash"] = readHashFile(
            join(buildRoot, spec["package"], ".build_succeeded")
        )
        ...
        if spec["is_devel_pkg"]:
            ...
            call_ignoring_oserrors(
                symlink,
                spec["hash"],
                join(buildWorkDir, "BUILD", spec["package"] + "-latest"),
            )
            if develPrefix:
                call_ignoring_oserrors(
                    symlink,
                    ...
                    join(
                        buildWorkDir,
                        "BUILD",
                        spec["package"] + "-latest-" + develPrefix,
                    ),
                )
        ...
        if fileHash == spec["hash"] and not spec["is_devel_pkg"]:
            ...
            debug(
                "Package %s was correctly compiled. Moving to next one.",
                spec["package"],
            )
            ...
            if not spec["is_devel_pkg"] and args.autoCleanup:
                cleanupDirs = [buildRoot, join(workDir, "INSTALLROOT", spec["hash"])]
                    ...
                    if "develPrefix" in args:
                        unlink(
                            join(
                                buildWorkDir,
                                "BUILD",
                                spec["package"] + "-latest-" + args.develPrefix,
                            )
                        )
                except Exception:
                    pass
        ...
        tar_hash_dir = os.path.join(
            workDir, resolve_store_path(args.architecture, spec["hash"])
        )
        ...
        if not spec["is_devel_pkg"]:
            ...
            debug(
                "Found tarball in %s" % spec["cachedTarball"]
                if spec["cachedTarball"]
                else "No cache tarballs found"
            )
        ...
        writeAll(
            "%s/build.sh" % scriptDir,
            cmd_raw
            % {
                "provenance": create_provenance_info(spec["package"], specs, args),
                "initdotsh_deps": generate_initdotsh(
                    p, specs, args.architecture, post_build=False
                ),
                "initdotsh_full": generate_initdotsh(
                    p, specs, args.architecture, post_build=True
                ),
        ...
        buildEnvironment += [
            (key, value) for key, value in spec.get("track_env", {}).items()
        ]
        ...
        if args.docker:
            build_command = (
                ...
                additionalEnv=" ".join(
                    "-e {}={}".format(var, quote(value))
                    for var, value in buildEnvironment
                ),
                ...
                mirrorVolume=(
                    "-v %s:/mirror" % quote(dirname(spec["reference"]))
            ...
            env_vars = " ".join(
                ["%s=%s" % (key, quote(val)) for key, val in buildEnvironment]
            )
            build_command = "env %s %s -e -x %s/build.sh 2>&1" % (
                env_vars,
                BASH,
                quote(scriptDir),
            )
        ...
        if not args.makeflow:
            if args.builders == 1:
                ...
                build_deps = [
                    "build:%s" % d
                    for d in specs[p]["full_requires"]
                    if d in buildTargets
                ]
                scheduler.parallel(
                    "build:%s" % p,
                    build_deps,
            ...
            breq = " ".join(
                [
                    str(element) + ".build"
                    for element in spec["full_requires"]
                    if element in buildTargets
                ]
            )
    ...
    if not args.onlyDeps:
        banner(
            f"Build of {mainPackage} successfully completed on `{socket.gethostname()}'.\n"
        ...
        banner(
            "Successfully built dependencies for package %s on `%s'.\n",
            mainPackage,
            socket.gethostname(),
        )
    for spec in specs.values():
        if spec["is_devel_pkg"]:
            banner(
                "Build directory for devel package %s:\n%s/BUILD/%s-latest%s/%s",
                spec["package"],
                abspath(buildWorkDir),
                spec["package"],
    ...
    if untrackedFilesDirectories:
        banner(
            "Untracked files in the following directories resulted in a rebuild of "
