...
def create_deps_info(package, specs, args):
  ...
  Built from the specs this build used, so it describes what was actually
  built rather than what the recipes say later. Never hashed.
  """
...
def create_provenance_info(package, specs, args):
  ...
  return json.dumps({
    ...
    # Runtime edges between the members of the closure above, as
    # {"pkg": ["dep", ...]} in build order (the same graph
    # `bits deps --outmake --runtime-only` prints). The lists above give the
    # closure but not the edges among its members, so this is what lets a
    # consumer derive the install order from this file alone. Never hashed.
