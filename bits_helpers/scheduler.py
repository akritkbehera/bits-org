...
class Scheduler(object):
    ...
    def __init__(
        self, parallelThreads, logDelegate=None, buildStats=None, parallelDownloads=2
    ):
        ...
        self.runningJobsCount = {
            "build": 0,
            "fetch": 0,
            "download": 0,
            "max_build": parallelThreads,
            "max_download": parallelDownloads,
        }
        self.errors = {}
        if buildStats:
        ...
        self.finalJobDeps = []
        self.finalJobSpec = [self.doSerial, "final-job", self.finalJobDeps] + [
            self.__doLog,
        ...
        self.jobs["final-job"] = {
            "scheduler": "serial",
            "deps": self.finalJobSpec,
            "spec": self.finalJobSpec,
        }
    ...
    def __doRescheduleParallel(self):
        parallelJobs = [
            j for j in self.pendingJobs if self.jobs[j]["scheduler"] == "parallel"
        ]
        # First of all clean up the pending parallel jobs from all those
        # which have broken dependencies.
        for taskId in parallelJobs:
            brokenDeps = [
                dep for dep in self.jobs[taskId]["deps"] if dep in self.brokenJobs
            ]
            if not brokenDeps:
                continue
            transition(taskId, self.pendingJobs, self.brokenJobs)
            self.errors[taskId] = (
                "The following dependencies could not complete:\n%s"
                % "\n".join(brokenDeps)
            )
        ...
        for taskId in parallelJobs:
            pendingDeps = [
                dep for dep in self.jobs[taskId]["deps"] if not dep in self.doneJobs
            ]
            if pendingDeps:
                continue
        ...
        dwnCount = (
            self.runningJobsCount["max_download"] - self.runningJobsCount["download"]
        )
        for task in sorted(allJobs, key=lambda k: k["priorty"]):
            ...
            if taskType == "download":
                ...
                if (
                    bldCount > 0
                ):  # include all build jobs so that we can match those which can be run
                    buildJobs.append(taskId)
            else:
                forceJobs.append(taskId)
        if bldCount > 0 and buildJobs:
            if self.resourceManager:
                buildJobs = self.resourceManager.allocResourcesForExternals(
                    buildJobs, count=bldCount
                )
            else:
        ...
        for taskId in forceJobs + downloadJobs + buildJobs:
            ...
            self.__scheduleParallel(
                taskId, self.jobs[taskId]["spec"], priorty=self.jobs[taskId]["priorty"]
            )
    ...
    def forceDone(self, taskId):
        if taskId in self.doneJobs:
            return
        if not taskId in self.jobs:
            self.jobs[taskId] = {}
        if not taskId in self.pendingJobs:
            self.pendingJobs.append(taskId)
            ...
            if [dep for dep in pendingDeps if not dep in brokenDeps]:
                self.resultsQueue.put(
                    (
                        ...
                        [self.doSerial, taskId, deps] + list(commandSpec),
                    )
                )
                return
            transition(taskId, self.pendingJobs, self.brokenJobs)
