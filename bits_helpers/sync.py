...
class HttpRemoteSync:
    ...
    def getRetry(
        self, url, dest=None, returnResult=False, log=True, session=None, progress=debug
    ):
            ...
            try:
                ...
                if dest or returnResult:
                    ...
                    if s3Request:
                        ...
                        resp = get(
                            url, verify=not self.insecure, timeout=self.httpTimeoutSec
                        )
                        ...
                        return [
                            {"name": os.path.basename(x), "type": "file"}
    ...
    def fetch_tarball(self, spec) -> None:
        ...
        for pkg_hash in spec["remote_hashes"]:
            ...
            for tarball in have_tarballs:
                if re.match(
                    r"^{package}-{version}-[0-9]+\.{arch}\.tar\.gz$".format(
        ...
        with requests.Session() as session:
            debug(
                "Updating remote store for package %s; trying hashes %s",
                spec["package"],
            ...
            for pkg_hash in spec["remote_hashes"]:
                ...
                tarballs = self.getRetry(
                    "%s/%s/" % (self.remoteStore, store_path), session=session
                )
                if tarballs:
            ...
            if not os.path.isfile(destPath):  # do not download twice
                progress = ProgressPrint(
                    "Downloading tarball for %s@%s"
                    % (spec["package"], spec["version"]),
                    min_interval=5.0,
                )
                progress(
                    "[0%%] Starting download of %s", use_tarball
                )  # initialise progress bar
                self.getRetry(
                    "/".join((self.remoteStore, store_path, use_tarball)),
                    destPath,
                    session=session,
                    progress=progress,
                )
    ...
    def fetch_symlinks(self, spec) -> None:
        ...
        if any(
            f"/{pkg_hash[:2]}/{pkg_hash}/" in target
            for target in (
                os.readlink(os.path.join(self.workdir, links_path, link))
                for link in os.listdir(os.path.join(self.workdir, links_path))
            )
            for pkg_hash in spec["remote_hashes"]
        ):
            debug(
                "Found symlink for %s@%s, not updating",
                spec["package"],
                spec["version"],
            )
            return
        ...
        with requests.Session() as session:
            ...
            manifest = self.getRetry(
                "%s/%s.manifest" % (self.remoteStore, links_path),
                returnResult=True,
                session=session,
            )
            symlinks = {
            ...
            for link in self.getRetry(
                "%s/%s/" % (self.remoteStore, links_path), session=session
            ):
                linkname = link["name"]
...
class RsyncRemoteSync:
    ...
    def fetch_tarball(self, spec) -> None:
        info(
            "Downloading tarball for %s@%s, if available",
            spec["package"],
            spec["version"],
        )
        debug(
            "Updating remote store for package %s with hashes %s",
            spec["package"],
            ", ".join(spec["remote_hashes"]),
        )
        err = execute(
    ...
    def fetch_symlinks(self, spec) -> None:
        ...
        os.makedirs(os.path.join(self.workdir, links_path), exist_ok=True)
        err = execute(
            "rsync -rlvW --delete {remote_store}/{links_path}/ {workdir}/{links_path}/".format(
...
class CVMFSRemoteSync:
    ...
    def fetch_tarball(self, spec) -> None:
        info(
            "Downloading tarball for %s@%s-%s, if available",
            spec["package"],
            spec["version"],
            spec["revision"],
        )
        ...
        for pkg_hash in spec["remote_hashes"] + spec["local_hashes"]:
            ...
            pattern = os.path.join(
                self.workdir, store_path, "%s-*.tar.gz" % spec["package"]
            )
            if glob.glob(pattern):
                info("Reusing existing tarball for %s@%s", spec["package"], pkg_hash)
                return
        info(
            "Could not find prebuilt tarball for %s@%s-%s, will be rebuilt",
            spec["package"],
            spec["version"],
            spec["revision"],
        )
    ...
    def fetch_symlinks(self, spec) -> None:
        ...
        info(
            "Fetching available build hashes for %s, from %s",
            spec["package"],
            self.remoteStore,
        )
        links_path = resolve_links_path(self.architecture, spec["package"])
        ...
        cvmfs_architecture = re.sub(
            r"slc(\d+)_x86-64", r"el\1-x86_64", self.architecture
        )
        err = execute(
            r"""\
    set -x
...
class Boto3RemoteSync:
    ...
    def _s3_init(self) -> None:
        ...
        try:
            ...
            error(
                "you must pass the AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY env "
    ...
    def _s3_listdir(self, dirname):
        ...
        pages = self.s3.get_paginator("list_objects_v2").paginate(
            Bucket=self.remoteStore, Delimiter="/", Prefix=dirname.rstrip("/") + "/"
        )
    ...
    def fetch_tarball(self, spec) -> None:
        ...
        for pkg_hash in spec["remote_hashes"]:
            ...
            for tarball in self._s3_listdir(store_path):
                ...
                progress(
                    "[0%%] Starting download of %s", tarball
                )  # initialise progress bar
                ...
                self.s3.download_file(
                    ...
                    Filename=os.path.join(
                        self.workdir, store_path, os.path.basename(tarball)
                    ),
                    Callback=lambda num_bytes: progress(
                        "[%d/%d] bytes transferred", num_bytes, total_size
                    ),
                )
                progress.end("done")
                return
        ...
        debug(
            "Remote has no tarballs for %s with hashes %s",
            spec["package"],
            ", ".join(spec["remote_hashes"]),
        )
    ...
    def fetch_symlinks(self, spec) -> None:
        ...
        n_symlinks = 0
        try:
            manifest = self.s3.get_object(
                Bucket=self.remoteStore, Key=links_path + ".manifest"
            )
            ...
            for line in manifest["Body"].iter_lines():
                ...
                link_path = os.path.join(
                    self.workdir, links_path, os.fsdecode(link_name)
                )
    ...
    def upload_symlinks_and_tarball(self, spec) -> None:
        ...
        for link_dir in ("dist", "dist-direct", "dist-runtime"):
            link_dir = "TARS/{arch}/{link_dir}/{package}/{package}-{version}-{revision}".format(
                arch=self.architecture, link_dir=link_dir, **spec
            )
        ...
        tarball = "{package}-{version}-{revision}.{architecture}.tar.gz".format(
            architecture=self.architecture, **spec
        )
        tar_path = os.path.join(
            resolve_store_path(self.architecture, spec["hash"]), tarball
        )
        link_path = os.path.join(
            resolve_links_path(self.architecture, spec["package"]), tarball
        )
        ...
        debug(
            "Uploading tarball and symlinks for %s %s-%s (%s) to S3",
            spec["package"],
            spec["version"],
            spec["revision"],
            spec["hash"],
            ...
            for link_dir, symlinks in dist_symlinks.items():
                for link_key, hash_path in symlinks:
                    future = executor.submit(
                        _upload_single_symlink, link_key, hash_path
                    )
        ...
        end_time = time.time()
        debug(
            "Uploaded %d dist symlinks in %.2f seconds",
            total_symlinks,
            end_time - start_time,
        )
