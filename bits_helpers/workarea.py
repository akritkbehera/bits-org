...
def cleanup_git_log(referenceSources):
    ...
    try:
        ...
        dieOnError(
            exc.errno != errno.ENOENT, "Could not delete stale git log: %s" % exc
        )


def logged_scm(
    scm, package, referenceSources, command, directory, prompt, logOutput=True
):
    ...
    if err:
        try:
            with codecs.open(
                os.path.join(referenceSources, FETCH_LOG_NAME),
                "a",
                encoding="utf-8",
                errors="replace",
            ) as logf:
                logf.write(
                    "%s command for package %r failed.\n"
                    "Command: %s %s\nIn directory: %s\nExit code: %d\n"
                    % (
                        scm.name,
                        package,
    ...
    dieOnError(
        err,
        "Error during %s %s for reference repo for %s."
...
def updateReferenceRepoSpec(
    referenceSources, p, spec, fetch=True, usePartialClone=True, allowGitPrompt=True
):
    """
    ...
    """
    spec["reference"] = updateReferenceRepo(
        referenceSources, p, spec, fetch, usePartialClone, allowGitPrompt
    )
    if not spec["reference"]:
        del spec["reference"]


def updateReferenceRepo(
    referenceSources, p, spec, fetch=True, usePartialClone=True, allowGitPrompt=True
):
    """
    ...
    if not os.path.exists(referenceRepo):
        ...
        cmd = scm.fetchCmd(
            spec["source"], "+refs/tags/*:refs/tags/*", "+refs/heads/*:refs/heads/*"
        )
...
def checkout_sources(spec, work_dir, reference_sources, containerised_build):
    ...
    source_parent_dir = os.path.join(
        work_dir, "SOURCES", spec["package"], spec["version"]
    )
    ...
    if spec["commit_hash"] != spec["tag"]:
        symlink(
            spec["commit_hash"],
            os.path.join(source_parent_dir, spec["tag"].replace("/", "_")),
        )
    ...
    if "patches" in spec:
        ...
        for patch in spec["patches"]:
            shutil.copyfile(
                os.path.join(spec["pkgdir"], "patches", patch),
                os.path.join(source_dir, patch),
            )
    if "sources" in spec:
        ...
        symlink(
            "/" + os.path.basename(spec["source"])
            if containerised_build
            else spec["source"],
            source_dir,
        )
        ...
        scm_exec(
            scm.cloneSourceCmd(
                spec["source"], source_dir, spec.get("reference"), usePartialClone=True
            )
        )
