...
def checkPreferSystem(spec, cmd, homebrew_replacement, getstatusoutput_docker):
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"bits_prefer_check_{spec['package']}_"
    ) as temp_dir:
    ...
    warning(
        "Package %s cannot be picked up from the system and will be built by bits.\n"
...
def checkRequirements(spec, cmd, homebrew_replacement, getstatusoutput_docker):
    ...
    if not err:
        success(
            "Required package %s will be picked up from the system.", spec["package"]
        )
        debug("%s", cmd)
        for x in out.split("\n"):
            debug("%s: %s", spec["package"], x)
...
def doDoctor(args, parser):
    ...
    with DockerRunner(
        ...
        fromSystem, own, failed, validDefaults = getPackageList(
            packages=packages,
            specs=specs,
            ...
            performPreferCheck=lambda pkg, cmd: checkPreferSystem(
                pkg, cmd, homebrew_replacement, getstatusoutput_docker
            ),
            performRequirementCheck=lambda pkg, cmd: checkRequirements(
                pkg, cmd, homebrew_replacement, getstatusoutput_docker
            ),
    ...
    if alwaysBuilt:
        banner(
            "The following packages will be built by bits because\n"
    ...
    if own:
        banner(
            "The following packages will be built by bits because they couldn't be picked up from the system:\n\n"
            "- %s\n\n"
